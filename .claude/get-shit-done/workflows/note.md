...
- **Global scope**: `/Users/quantumcode/CODE/CARPETIFY/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/quantumcode/CODE/CARPETIFY/.claude/notes/`)
...
2. Glob `/Users/quantumcode/CODE/CARPETIFY/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/quantumcode/CODE/CARPETIFY/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/quantumcode/CODE/CARPETIFY/.claude/notes/` — works in any directory
